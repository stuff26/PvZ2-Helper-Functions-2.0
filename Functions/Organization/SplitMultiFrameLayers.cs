...
{
    public static class SplitMultiFrameLayers
    {
        public static void Function()
        {
            ...
            {
                var Layers = symbol.Timeline.Layers; // Get layers
                var NewLayerList = SplitLayers(Layers); // Split layers
                NewLayerList = AddEmptyFrames(NewLayerList); // Add empty frames to beginning of layers 
                NewLayerList = NewLayerList.Where(l => !l.IsEmpty() && l.HasFrameElements()).ToList(); // Remove empty layers
                symbol.Timeline.ReplaceLayers(NewLayerList); // Replace layers
                editSymbols.AddOne();
            }
            ...
            {
                XmlMethods.SaveXmlDocument(SymbolPathList[i], SymbolList[i], SymbolItem.serializer);
                writeFiles.AddOne();
            }
        }
        ...
        public static (List<string> SymbolPathList, List<SymbolItem> SymbolList) AskForSymbolItem()
        {
            ...
            {
                // Get input from user
                var (pathInput, isFile) = UserPrompts.AskForPath(["DOMDocument.xml"]);
                ...
                if (!isFile)
                {
                    ...
                    if (!File.Exists(Path.Join(pathInput, "DOMDocument.xml")))
                    {
        ...
        private static List<AnimateLayer> SplitLayers(List<AnimateLayer> Layers)
        {
            ...
            foreach (var layer in Layers)
            {
                ...
                string? currentSymbol = string.Empty; // Current symbol that will be expected
                ...
                foreach (var frame in frames)
                {
                    ...
                    if (mainLibraryItem == string.Empty && currentSymbol != string.Empty)
                    {
                    ...
                    {
                        if (frame.GetMainLibraryItem() is not null)
                        {
